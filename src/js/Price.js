...
import { delUslugi1, delUslugiprice1, getKomplexpredlinfodesc, getUslugi_price, putKategorii, putPrice } from "../https/deviceAPI";
import { useEffect } from "react";
import useState from 'react-usestateref'
import $ from 'jquery'
import {Form} from "react-bootstrap";
export const Price = ({i,ids,setUslugi1}) => {
    ...
    useEffect(() => {
      // setprice(12123)
      // setname('Новое значение')
    })

    const function0111 = async() => {
    
      const dd = await getUslugi_price(ids)
  ...
  const function62 = async() => {
    ...
    $(`.class3${i.id}`).toggleClass('input_active')
    $(`.class2${i.id}`).toggleClass('input_1_active')
...
<div class= {`but_price input_2sd11${i.id} reveal fade-left`} >
...
<div class='dfl_l'>
<div class= {`class2 class2${i.id}`} >{i.name}</div>
<div class={`class2 class2${i.id}`} >от {i.price}р</div>

<input class= {`class3 title_tex_ class3${i.id}`}  defaultValue={i.name} onChange={e => setname(e.target.value) } placeholder={'Иконка'} type="text" />
<input class= {`class3  title_tex_1 title_tex_11 class3${i.id}`} defaultValue={i.price} onChange={e => setprice(e.target.value) } placeholder={'Иконка'} type="number" />

</div>
<button class= {`class3 mt_2_ class3${i.id}`} onClick={function6}>сохранить</button>
<button class= {`class3 mt_2_ class3${i.id}`} onClick={del_price}>удалить</button>
</div>
