...
import { delActiiinfo1, getKomplexpredlinfodesc, putActii_info, putKategorii, putUslugi } from "../https/deviceAPI";
...
export const Actii1 = ({i,get}) => {
    ...
    $(`.section__title_2${i.id}`).toggleClass('input_1_active')
      ...
      const function625 = async() => {
    
        $(`.class12`).removeClass('input_active')
        $(`.section__title_2`).removeClass('input_1_active')
    ...
    const function6 = async() => {
        ...
        get()
         ...
         const del_kateg = async() => {
            await delActiiinfo1(i.id)
          $(`.input_as21ad1${i.id}`).addClass('ddd_nnn')
    ...
    return (
     <div class={`input_as21ad1${i.id} reveal fade-right`}>
  ...
  <div class="dffd ">
   ...
   <div class='dddff_ds'>
   <button onClick={function6} class={`bububu ma_le`} >сохранить</button>
    <button onClick={del_kateg} class={`bububu ma_le`} >удалить</button>
   </div>
